...
import { Manager, Manager__factory, MockLink, MockLink__factory, MockNFT, MockNFT__factory, MockToken, MockToken__factory } from "../types";
...
describe("Raffle Manager test", () => {
  ...
  it("create a raffle", async () => {
    ...
    await expect(manager.createRaffle(createParams, prices, [])).to.be.revertedWith(`AccessControl: account ${deployer.address.toLowerCase()} is missing role ${OPERATOR_ROLE.toLowerCase()}`);

    let tx = await manager.connect(alice).createRaffle(createParams, prices, []);
    let receipt = await tx.wait();
    let expectKey = solidityKeccak256(["uint8", "address", "uint256", "uint"], [RaffleType.NFT, nft.address, 1, receipt.blockNumber])
    ...
    tx = await manager.connect(alice).createRaffle({ ...createParams, raffleType: RaffleType.ETH, collateralAddress: constants.AddressZero, collateralParam: parseEther("10") }, prices, []);
    receipt = await tx.wait();
    expectKey = solidityKeccak256(["uint8", "address", "uint256", "uint"], [RaffleType.ETH, constants.AddressZero, parseEther("10"), receipt.blockNumber])
    ethRaffleKey = expectKey;

    tx = await manager.connect(alice).createRaffle({ ...createParams, raffleType: RaffleType.ERC20, collateralAddress: token.address, collateralParam: parseEther("10") }, prices, []);
    receipt = await tx.wait();
    expectKey = solidityKeccak256(["uint8", "address", "uint256", "uint"], [RaffleType.ERC20, token.address, parseEther("10"), receipt.blockNumber])
    erc20RaffleKey = expectKey;
  });
  ...
  it("stake nft test", async () => {
    ...
    await nft.mint(1);
    await expect(manager.stakeNFT(nftRaffleKey)).to.be.revertedWith('ERC721: caller is not token owner or approved');
  ...
  it("stake token test", async () => {
    ...
    await expect(manager.stakeERC20(erc20RaffleKey)).to.be.revertedWith('ERC20: transfer amount exceeds balance');
